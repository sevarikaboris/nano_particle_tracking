...
for experiment_dir in os.listdir(basepath):
    ...
    counter_fov = 0
    for videos_fov_dir in os.listdir(basepath + experiment_dir):
        if(not os.path.isdir(basepath + experiment_dir + videos_fov_dir)):
            continue
    ...
    for concat_row in concat_ptcl2:
        ...
        img2Name = basepath + experiment_dir + video_folder_list[int(concat_row[2])-1] + "/" +  video_folder_list[int(concat_row[2])-1] + number + '.tif'
